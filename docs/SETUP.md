...
### חבילת תמונות ממותגת ומובטחת
לפני האישור המוצר יוצר בסיס ויזואלי רלוונטי לנושא באמצעות GPT Image, ללא
טקסט, ואז מוסיף בעצמו את שם ד״ר גיא רופא ואת הכותרת בעברית. כך המלל הממותג
אינו תלוי ביכולת של מודל התמונה לכתוב עברית. בכל הרצה נוצרים hero, landscape,
square ו-portrait, עם ALT נגיש שמכיל פעם אחת את שם הלקוח ואת נושא המאמר.

אם שירות התמונה אינו זמין, מנוע מקומי דטרמיניסטי יוצר כרטיס ממותג מלא. אין
מסלול שממשיך לאישור בלי תמונה. `OPENAI_API_KEY` הקיים משמש גם ליצירת בסיס
התמונה; ברירת המחדל היא `OPENAI_IMAGE_MODEL=gpt-image-2`.
Wikimedia Commons נשארת אפשרות ידנית לתמונה מורשית, אך אינה נקודת כשל.
