...
### חבילת תמונות ללא מלל ומובטחת
לפני האישור המוצר מחפש קודם צילום רלוונטי ב-Wikimedia Commons, מאמת שהוא
צילום אמיתי, בודק התאמה לנושא ורישיון, ושומר את המקור והייחוס. אם לא נמצא
צילום מתאים, GPT Image יוצר תמונה ללא אותיות, כותרות או סימני מים. בכל הרצה
נוצרים hero, landscape, square ו-portrait, כולם ללא שכבת טקסט, עם ALT נגיש
שמכיל פעם אחת את שם הלקוח ואת נושא המאמר.

אם שירות התמונה אינו זמין, מנוע מקומי דטרמיניסטי יוצר תמונה מופשטת ללא מלל.
אין מסלול שממשיך לאישור בלי תמונה. `OPENAI_API_KEY` משמש לתכנון ולאימות החיפוש
וכן ליצירת AI כאשר אין צילום מורשה מתאים; ברירת המחדל היא
