...
### תמונות אמיתיות ומורשות
יצירת תמונות AI מושבתת. לפני האישור המוצר מחפש צילום קיים ב-Wikimedia
Commons, מוודא שהקובץ הוא צילום bitmap ברישיון שימוש מותר, בודק התאמה ישירה
לנושא ושומר בחבילת האישור את היוצר, דף המקור, הרישיון והקרדיט. אם אין צילום
מתאים, ההרצה נעצרת במקום להציג תמונה גנרית. לאחר האישור הצילום מועלה לספריית
המדיה של `guyrofe.com` ומשמש את הערוצים המאושרים עם alt text וקרדיט.

`OPENAI_API_KEY` משמש לתכנון שאילתת החיפוש ולבדיקת ההתאמה בלבד; הוא אינו
משמש ליצירת תמונה. אין צורך ב-Secret תמונות נוסף.
