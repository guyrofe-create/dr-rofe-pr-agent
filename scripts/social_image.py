"""Generate an authentic editorial photograph with OpenAI and host it on WordPress."""
...
def visual_description(title):
    ...
    return (
        "צילום עריכתי טבעי ואמין של סצנה יומיומית הקשורה "
...
def build_prompt(title, summary):
    ...
    return f"""
Create a premium square documentary editorial PHOTOGRAPH for an educational
Hebrew information post published by {_CLIENT_NAME}. It must look commissioned
and art-directed by a skilled human editorial photographer, not generated by
AI, not like stock illustration, and not like a social-media template.

Exact post title: {clean_title}
Article context (use this to understand the exact subject): {context}

Visual direction:
- depict ONE coherent, physically plausible real-world scene that is
  unmistakably and specifically related to the exact post title and context
- use at most two ordinary real objects that belong naturally in that scene;
  avoid a collection of symbolic props
- natural window light, believable shadows, realistic materials, subtle
  texture, restrained color grading and slight human imperfection
- documentary 50mm-lens feeling, medium-close framing and natural depth of field
- credible for an evidence-based newspaper or health magazine
- use navy or turquoise only as a small real-world accent when natural; never
  force a blue color wash over the whole image
- trustworthy, understated and visually specific rather than dramatic
...
- no illustration, vector art, collage, infographic, 3D render, CGI, glossy
  product mockup, surrealism or fantasy
- no floating objects, decorative leaves, abstract waves, perfect circles,
  split-screen composition, glowing areas, lightning bolts, pain rays, halos
  or icon-like symbols
- no uncanny anatomy, distorted or duplicated objects, impossible reflections,
  excessive symmetry, plastic surfaces or over-smoothed skin
- no unrelated generic medical imagery, random people, decorative charts or
  a visual concept that could fit a different article
{exclusions}
""".strip()


def review_relevance(client, image_bytes, title, summary):
    """Reject generic or visibly synthetic images; return truthful alt text."""
                ...
                "content": [
                    {
                        "type": "input_text",
                        "text": (
                            "Act as a strict senior editorial photo editor. Accept "
                            "this image only if it (1) clearly and specifically "
                            "represents the post, (2) reads as one believable "
                            "documentary photograph, and (3) has no obvious AI tells. "
                            "Reject generic health imagery and reject illustrations, "
                            "3D/CGI, icon collages, floating symbolic objects, glows, "
                            "lightning, decorative leaves or waves, excessive "
                            "symmetry, plastic textures, impossible geometry, "
                            "distorted anatomy, duplicated objects, oversmoothing, "
                            "or a scene that feels staged by an image generator.\n\n"
                            f"Title: {title}\n"
                            f"Context: {' '.join((summary or '').split())[:2400]}\n\n"
                            "Return exactly one line. If suitable: "
                            "ACCEPT: followed by a concrete, truthful Hebrew "
                            "description of what is visibly present, suitable as alt "
                            "text. If unsuitable: REJECT: followed by a short, "
                            "specific English correction for a substantially "
                            "different next attempt."
                        ),
                    },
                    {
                        "type": "input_image",
...
def generate(title, summary, client=None):
    ...
    for attempt in range(1, 4):
        ...
        if last_feedback:
            attempt_prompt += (
                "\n\nA senior photo editor rejected the previous image. Create a "
                "substantially different, more authentic photographic scene; do "
                "not merely rearrange or restyle the same objects. Correction: "
                f"{last_feedback}"
            )
    ...
    *,
    ...
    metadata = requests.post(
        ...
        json={
            ...
            "caption": "תמונה עריכתית שנוצרה באמצעות OpenAI",
        },
        headers=headers,
        timeout=30,
    )
